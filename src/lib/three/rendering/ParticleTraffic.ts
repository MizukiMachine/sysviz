...
type PointLike = THREE.Vector3 | { x: number; y: number; z: number };

interface TrafficRoute extends VisualizationRoute {
  id: string;
  sourceId: string;
  targetId: string;
  sourcePos?: PointLike;
...
export class ParticleTrafficSystem {
  ...
  labelSprites!: Array<THREE.Sprite>;
  spawnTimers: Map<string, number>;
  _tmpVec: THREE.Vector3;
  ...
  _getLabelTexture(text: string): THREE.CanvasTexture | null {
    const label = text || '';
  ...
  _setParticleLabel(index: number, particle: Particle, route: StoredRoute | undefined): void {
  ...
  addRoute(route: VisualizationRoute): void {
    const trafficRoute = route as TrafficRoute;
    if (this.routes.has(trafficRoute.id)) return;

    const curve = this._buildCurve(trafficRoute);
    if (!curve) return;
    ...
    this.routes.set(trafficRoute.id, {
      ...
      color: TRAFFIC_COLORS[trafficRoute.trafficType as keyof typeof TRAFFIC_COLORS] || TRAFFIC_COLORS.default,
      active: false,
    });

    this.spawnTimers.set(trafficRoute.id, 0);
  }
  ...
  updateRoute(routeId: string, updates: Partial<TrafficRoute>): void {
  ...
  _toVector3(pos: PointLike): THREE.Vector3 {
  ...
  _deactivateParticle(particle: Particle): void {
  ...
  _spawnParticle(route: StoredRoute): void {
  ...
  _spawnNewParticles(delta: number): void {
  ...
  _updateActiveParticles(delta: number): void {
    ...
    for (let i = 0; i < POOL_SIZE; i++) {
      ...
      const route = this.routes.get(particle.routeId as string);
