...
- 全長: 8.15秒
- 24キーフレーム
- 各セグメント: ノードactive → route開始 → route終了 → 次ノードactive + 前ノードcomplete

### タイミング（Flask 1リクエスト）
```
0.0s   Browser(request) active
0.4s   Browser→WSGI パーティクル開始
1.3s   WSGI active, Browser complete
1.75s  WSGI→Routing パーティクル開始
2.65s  Routing active, WSGI complete
3.1s   Routing→View パーティクル開始
4.0s   View active, Routing complete
4.45s  View→Response パーティクル開始
5.35s  Response active, View complete
5.8s   Response→Browser(render) パーティクル開始
6.9s   Browser(render) active, Response complete
8.15s  終了
```
...
1. **完了**: リニア配置 + 6ノードFlask可視化 + 形状バリエーション + 接続タイプ
